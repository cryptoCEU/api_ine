...
/** Filter series by exact municipality code */
...
/** Filter series by province code (2-digit prefix match) */
export function filterByProv(series, provCode) {
  ...
  return series.filter(s =>
    (s.MetaData || []).some(m => {
      ...
      // Match exact province code (2 digits) or first-level aggregation
      return c === prov2.padStart(5, '0') ||
             c.startsWith(prov2) ||
...
/**
 * Try municipality first; if empty, fall back to province.
 * Returns { data: Series[], level: 'muni' | 'prov' | 'none' }
 */
export function filterWithFallback(series, muniCode, provCode) {
  const muniData = filterByMuni(series, muniCode)
  if (muniData.length > 0) {
    console.log(`[filter] municipio OK: ${muniData.length} series`)
    return { data: muniData, level: 'muni' }
  }
  console.warn(`[filter] sin datos de municipio, intentando provincia ${provCode}`)
  const provData = filterByProv(series, provCode)
  if (provData.length > 0) {
    console.log(`[filter] provincia OK: ${provData.length} series`)
    return { data: provData, level: 'prov' }
  }
  console.warn(`[filter] sin datos de provincia — sin resultados`)
...
/**
 * Get all available years (no limit), sorted ascending.
 * Pass maxYears to cap if needed.
 */
