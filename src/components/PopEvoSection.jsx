...
export default function PopEvoSection({ muniCode, muniName, provCode, provName, ccaaCode, ccaaName }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // No nult → fetch ALL available years from INE
      ...
      // Prefer total (not broken down by sex)
      ...
      const pts = getTimeSeries([totalS]) // ALL years
  ...
  const varData    = chartData.slice(1).map((d,i) => ({ año: d.año, variacion: Math.round(d.value - chartData[i].value) }))
  const yearsCount = chartData.length > 1 ? chartData.length - 1 : 0
  const rangeLabel = yearsCount > 0 ? `${chartData[0]?.año}–${chartData[chartData.length-1]?.año} (${yearsCount} años)` : 'todos los años disponibles'

  return (
    <Section title="Evolución de la población" badge={`Padrón Municipal · ${rangeLabel}`} icon="📈">
      {loading && <Spinner />}
      {!loading && error && <ErrorBox msg={error} retry={load} href="https://www.ine.es/jaxiT3/Tabla.htm?t=2879"/>}
      {!loading && !error && (
        <>
          {/* ← fix: show FallbackBadge for both 'prov' and 'ccaa' */}
          {dataLevel !== 'muni' && (
