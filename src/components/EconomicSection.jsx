...
export function BusinessSection({ muniCode, muniName, provCode, provName }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!series.length) throw new Error('Sin datos para este municipio ni provincia')
...
export function LaborSection({ muniCode, muniName, provCode, provName }) {
  ...
  useEffect(() => {
    ...
    ineGet('DATOS_TABLA/13930', { nult: 1 })
      .then(all => {
        const { data: s, level } = filterWithFallback(all, muniCode, provCode)
        const v = getLatest(s)
        if (v != null) { setSalary(v); setLevel(level) }
        else setError('Sin datos de salario para este municipio ni provincia')
      })
      .catch(e => setError(e.message))
      .finally(() => setLoading(false))
  }, [muniCode, provCode])
