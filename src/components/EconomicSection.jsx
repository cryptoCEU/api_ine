...
import { ineGet, filterByMuni, getLatest, fNum, fEur, CH } from '../utils'
import { Section, Spinner, ErrorBox, KPI, Grid, ChartTooltip, Source } from './UI'

// ─── 2.3.1 Distribución por actividad económica ──────────────────────────────
export function BusinessSection({ muniCode }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!series.length) throw new Error('Sin datos para este municipio')
        ...
        <>
          {total != null && (
            <div style={{marginBottom:'1rem'}}>
              <KPI label="Total empresas" value={fNum(total)}/>
            </div>
          )}
...
export function LaborSection({ muniCode }) {
  ...
  useEffect(() => {
    ...
    ineGet('DATOS_TABLA/13930', { nult: 1 })
      .then(all => {
        const s = filterByMuni(all, muniCode)
        const v = getLatest(s)
        if (v != null) setSalary(v)
        else setError('Sin datos de salario para este municipio')
      })
      .catch(e => setError(e.message))
      .finally(() => setLoading(false))
  }, [muniCode])

  const natRef = 25897 // salario bruto medio España (Encuesta Estructura Salarial)
        ...
        <div>
          ...
          <div style={{marginTop:10,fontSize:12,color:'var(--text2)'}}>
            ...
            <a href="https://www.sepe.es/HomeSepe/que-es-el-sepe/estadisticas/datos-estadisticos/municipios.html"
              target="_blank" rel="noreferrer">Paro y contratos SEPE →</a>
            {' · '}
            <a href="https://w6.seg-social.es/PXWeb/pxweb/es/Afiliados%20en%20alta%20laboral/"
              target="_blank" rel="noreferrer">Afiliados Seg. Social →</a>
          </div>
        </div>
      )}
      {!loading && !error && salary && (
        <>
          <Grid cols={3}>
            ...
            <KPI label="Referencia nacional"    value={fEur(natRef)} sub="España media"/>
            <KPI label="vs. media nacional"     value={`${salary > natRef ? '+' : ''}${fNum((salary/natRef-1)*100, 1)}%`}
              color={salary >= natRef ? CH.teal : CH.red}/>
          </Grid>
          <div style={{marginTop:8,padding:'0.75rem',background:'var(--bg3)',borderRadius:'var(--radius-sm)',fontSize:12,color:'var(--text2)'}}>
            ...
            <a href="https://www.sepe.es/HomeSepe/que-es-el-sepe/estadisticas/datos-estadisticos/municipios.html"
              target="_blank" rel="noreferrer">Tasa de paro y contratos SEPE</a>
            {' · '}
            <a href="https://w6.seg-social.es/PXWeb/pxweb/es/Afiliados%20en%20alta%20laboral/"
              target="_blank" rel="noreferrer">Afiliados por sector Seg. Social</a>
          </div>
