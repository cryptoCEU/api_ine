...
const LEVEL_CONFIG = {
  prov: { icon:'🟡', bg:'var(--warn-bg)', border:'#E8C84A', text:'var(--warn)',   label:'datos provinciales'  },
  ccaa: { icon:'🟠', bg:'#FFF0E0',        border:'#E8A84A', text:'#A05A00',       label:'datos de la CCAA'    },
}

export function FallbackBadge({ level, muniName, provName, ccaaName }) {
  const cfg = LEVEL_CONFIG[level]
  if (!cfg) return null
  const origin = level === 'prov'
    ? `la provincia de ${provName}`
    : `la Comunidad Autónoma de ${ccaaName}`
  return (
    <div style={{background:cfg.bg,border:`1px solid ${cfg.border}`,borderRadius:'var(--radius-sm)',padding:'8px 12px',fontSize:12,color:cfg.text,marginBottom:12,display:'flex',alignItems:'flex-start',gap:8}}>
      <span>{cfg.icon}</span>
      <div>
        <b>Sin datos para {muniName} — mostrando {cfg.label}</b>
        <div style={{marginTop:2,opacity:.85}}>Los datos mostrados corresponden a <b>{origin}</b>, no al municipio exacto.</div>
      </div>
    </div>
  )
}
...
export function KPI({ label, value, sub, color }) {
  return (
    <div style={{background:'var(--bg3)',borderRadius:'var(--radius-sm)',padding:'1rem'}}>
      <div style={{fontSize:11,textTransform:'uppercase',letterSpacing:1.1,color:'var(--text3)',fontWeight:600,marginBottom:4}}>{label}</div>
...
export function Section({ title, badge, icon, children }) {
  return (
    <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--radius)',padding:'1.5rem',boxShadow:'var(--shadow)',animation:'fadeup .35s ease both'}} className="fade">
