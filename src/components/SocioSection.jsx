...
export default function SocioSection({ muniCode, muniName, provCode, provName }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (!renta) setError('No se encontraron datos de renta ni para el municipio ni para la provincia')
  ...
  const barData = useMemo(() => renta ? [
    { name: 'Municipio / Prov.', value: Math.round(renta) },
    { name: 'Mediana nacional',  value: NAT_MEDIAN        },
  ] : [], [renta])

  const showFallback = rentaLevel === 'prov' || gastoLevel === 'prov'

  return (
    <Section title="Perfil socioeconómico" badge="Atlas distribución de renta · INE" icon="💰">
      {loading && <Spinner />}
      {!loading && !renta && <ErrorBox msg={error || 'Sin datos de renta'} href="https://www.ine.es/jaxiT3/Tabla.htm?t=9949"/>}
      {!loading && renta && (
        <>
          {showFallback && <FallbackBadge muniName={muniName} provName={provName}/>}
          <Grid cols={3}>
