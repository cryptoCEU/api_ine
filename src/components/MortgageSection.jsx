...
import { Section, Spinner, KPI, Grid, ChartTooltip, Source, FallbackBadge } from './UI'
...
function maxPrice(monthlyCap, years, annualRate, ltv) {
  const r = annualRate / 100 / 12
  const n = years * 12
  if (r === 0) return monthlyCap * n / ltv
  return (monthlyCap * (Math.pow(1+r,n)-1) / (r*Math.pow(1+r,n))) / ltv
}

export default function MortgageSection({ muniCode, muniName, provCode, provName, ccaaCode, ccaaName }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (r1.status === 'fulfilled') {
        const { data: s, level } = filterWithFallback(r1.value, muniCode, provCode, ccaaCode) // ← fix: ccaaCode
        ...
        else setError('Sin datos de renta — usando estimación nacional')
      }
      if (r2.status === 'fulfilled') {
        const { data: s } = filterWithFallback(r2.value, muniCode, provCode, ccaaCode) // ← fix: ccaaCode
        const v = getLatest(s)
  ...
  const maxCuota     = baseRenta / 12 * 0.30
  ...
  const rateRange = useMemo(() => {
    ...
    return [-5,-3,-1,0,1,3,5].map(delta => {
      ...
      return { delta: delta===0?'Base':`${delta>0?'+':''}${delta}%`, rate:+rate.toFixed(2), precio, cuota }
    })
      ...
      {!loading && (
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem',marginBottom:'1.25rem',background:'var(--bg3)',borderRadius:'var(--radius-sm)',padding:'1rem'}}>
            {sliders.map(({ label, min, max, step, val, set, sub }) => (
              <div key={label}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={rateRange} margin={{left:0,right:10,top:4,bottom:0}}>
              ...
              <Line type="monotone" dataKey="precio" stroke={CH.blue} strokeWidth={2.5} dot={{r:4,fill:CH.blue}} name="Precio asumible"/>
            </LineChart>
          </ResponsiveContainer>
