...
import { Section, Spinner, ErrorBox, KPI, ChartTooltip, Source, FallbackBadge } from './UI'
...
export default function DemoSection({ muniCode, muniName, provCode, provName }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (r1.status === 'fulfilled') {
        ...
        if (s.length) {
          const pts = getTimeSeries([s[0]])
          setFluxData(pts.map(p => ({ año: String(p.year), variacion: p.value })))
          lvl.flux = level; anyData = true
        }
      }
      if (r2.status === 'fulfilled') {
        const { data: s, level } = filterWithFallback(r2.value, muniCode, provCode)
        ...
        if (espV != null || extV != null) {
          setNacData({ esp: espV || 0, ext: extV || 0 })
          lvl.nac = level; anyData = true
        }
      }
      if (r3.status === 'fulfilled') {
        const { data: s, level } = filterWithFallback(r3.value, muniCode, provCode)
        if (s.length) {
          setInmData(getTimeSeries([s[0]]).map(p => ({ año: String(p.year), value: p.value })))
          lvl.inm = level; anyData = true
        }
      }
      if (r4.status === 'fulfilled') {
        const { data: s, level } = filterWithFallback(r4.value, muniCode, provCode)
        if (s.length) {
          setEmigData(getTimeSeries([s[0]]).map(p => ({ año: String(p.year), value: p.value })))
          lvl.emig = level; anyData = true
        }
      }
      setLevels(lvl)
      if (!anyData) setError('Sin datos de dinámica demográfica para este municipio ni provincia.')
  ...
  const anyProv = Object.values(levels).some(l => l === 'prov')
  ...
  return (
    <Section title="Dinámicas demográficas y migraciones"
      ...
      {!loading && hasAny && (
        <>
          {anyProv && <FallbackBadge muniName={muniName} provName={provName}/>}

          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1.5rem'}}>
            {/* Variación residencial */}
            <div>
              <div style={{fontSize:12,color:'var(--text2)',fontWeight:600,marginBottom:8}}>
                Variación residencial neta{fluxData.length > 0 ? ` (${fluxData[0].año}–${fluxData[fluxData.length-1].año})` : ''}
              </div>
              {fluxData.length > 0
          ...
          {(inmData.length > 0 || emigData.length > 0) && (
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1.5rem',marginTop:'1.5rem'}}>
              {inmData.length > 0 && (
                <div>
                  <div style={{fontSize:12,color:'var(--text2)',fontWeight:600,marginBottom:8}}>
                    Inmigración exterior{inmData.length > 1 ? ` (${inmData[0].año}–${inmData[inmData.length-1].año})` : ''}
                  </div>
              ...
              {emigData.length > 0 && (
                <div>
                  <div style={{fontSize:12,color:'var(--text2)',fontWeight:600,marginBottom:8}}>
                    Emigración exterior{emigData.length > 1 ? ` (${emigData[0].año}–${emigData[emigData.length-1].año})` : ''}
                  </div>
                  <ResponsiveContainer width="100%" height={150}>
