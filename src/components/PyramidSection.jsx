...
import { Section, Spinner, ErrorBox, KPI, Grid, ChartTooltip, Source, InfoBadge, FallbackBadge } from './UI'

export default function PyramidSection({ muniCode, muniName, provCode, provName }) {
  const [data,      setData]    = useState(null)
  const [totals,    setTotals]  = useState(null)
  const [year,      setYear]    = useState(0)
  const [dataLevel, setLevel]   = useState('muni')   // 'muni' | 'prov' | 'estimated'
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Fetch ALL available data (no nult limit for pyramid — it's always current)
      ...
      if (series.length > 0) {
        ...
        if (th + tm > 0) {
          setData(pd); setTotals({ h: th, m: tm, total: th + tm })
          setYear(yr); setLevel(level)
          setLoading(false); return
        }
      }
      throw new Error('sin datos de pirámide')
    } catch {
      // Fallback: get total pop and estimate with national distribution
      try {
        const all2 = await ineGet('DATOS_TABLA/2879', { nult: 1 })
        const { data: s2, level } = filterWithFallback(all2, muniCode, provCode)
        const tot = getLatest(s2) || 20000
        const yr  = (s2[0]?.Data || []).sort((a, b) => (b.Anyo || 0) - (a.Anyo || 0))[0]?.Anyo || 2023
        buildEstimated(tot, yr)
        setLevel(level === 'prov' ? 'prov-estimated' : 'estimated')
      } catch {
        buildEstimated(20000, 2023)
      }
    } finally { setLoading(false) }
  }, [muniCode, provCode, parsePyramid, buildEstimated])
  ...
  const isEstimated = dataLevel === 'estimated' || dataLevel === 'prov-estimated'
  const isProv      = dataLevel === 'prov' || dataLevel === 'prov-estimated'
  ...
  return (
    <Section title="Pirámide de población" badge={`Padrón Municipal${year ? ` · ${year}` : ''}`} icon="👥">
      ...
      {!loading && (
        <>
          {isProv && <FallbackBadge muniName={muniName} provName={provName} />}
          {isEstimated && !isProv && (
            <InfoBadge>
              Distribución estimada con proporciones nacionales.{' '}
