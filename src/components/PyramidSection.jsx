...
import { ineGet, filterByMuni, getLatest, AGE_GROUPS, SPAIN_M, SPAIN_F, fNum } from '../utils'
import { Section, Spinner, ErrorBox, KPI, Grid, ChartTooltip, Source, InfoBadge } from './UI'

export default function PyramidSection({ muniCode, muniName }) {
  const [data, setData]       = useState(null)
  const [totals, setTotals]   = useState(null)
  const [year, setYear]       = useState(0)
  const [estimated, setEst]   = useState(false)
  const [loading, setLoading] = useState(true)
  const [error, setError]     = useState('')

  const buildEstimated = useCallback((totalPop, yr) => {
  ...
  const load = useCallback(async () => {
    if (!muniCode) return
    setLoading(true); setError(''); setEst(false)
    try {
      // Try padron municipal by age and sex — table 56934
      ...
      if (series.length > 0) {
        ...
        series.forEach(s => {
          ...
          const ag = AGE_GROUPS.find(g => {
            const parts = g.split('-')
            return ageNum === parseInt(parts[0])
          })
          if (!ag) return
        ...
        const th = pd.reduce((s, x) => s + x.absH, 0)
        const tm = pd.reduce((s, x) => s + x.absM, 0)
        if (th + tm > 0) {
          setData(pd); setTotals({ h: th, m: tm, total: th + tm }); setYear(yr)
          setLoading(false); return
        }
      }
      throw new Error('sin datos pirámide')
    } catch {
      // Fallback: get total pop and estimate distribution
      try {
        const all2 = await ineGet('DATOS_TABLA/2879', { nult: 1 })
        const s2 = filterByMuni(all2, muniCode)
        const tot = getLatest(s2) || 20000
        const yr  = (s2[0]?.Data || []).sort((a,b) => (b.Anyo||0)-(a.Anyo||0))[0]?.Anyo || 2023
        buildEstimated(tot, yr)
        setError('Distribución estimada con proporciones nacionales (datos exactos en INE)')
      } catch {
        buildEstimated(20000, 2023)
        setError('No se pudo obtener población — mostrando distribución tipo')
      }
    } finally { setLoading(false) }
  }, [muniCode, buildEstimated])
  ...
  const medianAge = useMemo(() => {
    if (!data) return null
    let cum = 0
    const total = data.reduce((s,d) => s + d.absH + d.absM, 0)
    for (const d of data) {
      cum += d.absH + d.absM
      if (cum >= total / 2) return parseInt(d.age) + 2
    }
    return null
  }, [data])
      ...
      {!loading && (
        <>
          {estimated && (
            <InfoBadge>
              {error}{' '}
              <a href="https://www.ine.es/aplicaciones/piramides/" target="_blank" rel="noreferrer">
                Ver pirámide oficial →
              </a>
            </InfoBadge>
          )}
          {totals && (
          ...
          {medianAge && (
            <div style={{fontSize:12,color:'var(--text2)',marginBottom:12}}>
              Edad mediana estimada: <b style={{color:'var(--text)'}}>{medianAge} años</b>
            </div>
          )}
